...
export function ScanButton({ onResult, disabled, className, label = "Сканирай баркод" }: Props) {
  ...
  useEffect(() => {
    ...
    // Push a history entry so the Android back button closes the scanner
    // instead of exiting the app/PWA.
    ...
    return () => {
      ...
      // If the scanner was closed via UI (not back button), remove the
      // history entry we pushed so the back button still works normally.
      if (!poppedByUs) {
        try {
          if (window.history.state && (window.history.state as any).scanner) {
            window.history.back();
          }
        } catch {}
      }
  ...
  return (
    <>
      <button
        ...
        className={
          "inline-flex items-center justify-center gap-2 rounded-xl px-4 py-3 text-sm font-medium shadow-sm hover:opacity-90 disabled:opacity-50 border " +
          (className ?? "")
        }
        style={{
